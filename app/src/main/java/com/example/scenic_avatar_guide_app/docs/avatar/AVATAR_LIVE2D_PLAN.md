...
```text
core/tts/RemoteTTSController.kt
ui/components/AvatarView.kt
core/avatar/          # Live2D 渲染与 JNI 桥接（原 com/live2d/demo 已整合）
app/src/main/cpp/
...
6. **后台返回瞬时恢复已完成**：保留 EGL context、纹理和 Native 模型单例，正常切应用/按 Home 返回时不再出现模型重新加载等待。详见第五节。
...
- `ui/components/AvatarView.kt`
- `app/src/main/cpp/LAppDelegate.cpp`
- `app/src/main/cpp/LAppLive2DManager.cpp`

#### 问题描述

应用切到后台一段时间后再回来，Live2D 区域会出现几秒钟的模型加载过程。根因不是 Kotlin 层 `loadModel()`，而是 `GLSurfaceView.onPause()` 后 EGL context/纹理被释放，恢复时重新触发 `onSurfaceCreated()`，Native 层需要执行 `ReloadAllRenderers()` / `SetupTextures()`，造成可见等待。

#### 解决方案

目标从“严格复刻官方样例生命周期”调整为“用户返回时无加载感”：

1. **保留 EGL context**：`Live2DGLSurfaceView.initialize()` 设置 `preserveEGLContextOnPause = true`。正常按 Home/切应用时，GL context 未被系统回收，返回不会重新触发 `onSurfaceCreated()` 和纹理重建。
2. **页面销毁不释放 Native 模型**：`AvatarView` 的 `DisposableEffect` 只调用 `renderer.detachSurfaceView()` 解绑回调，不再调用 `renderer.release()` / `nativeOnStop()` / `nativeOnDestroy()`。
3. **避免初始化时误 pause**：`DisposableEffect` 不依赖 `renderer` 变化，使用 `rememberUpdatedState(renderer)` 获取最新 renderer，避免 `renderer=null -> 实例` 时触发旧 effect 的 `onDispose()`。
4. **Native 单例可重建兜底**：`LAppDelegate`、`LAppLive2DManager` 使用 `s_instance == NULL` 创建，避免 release 后同进程内无法重新创建。
5. **首次加载优先**：`LAppDelegate::OnSurfaceCreate()` 首次加载时不提前删除 shader 单例；只有已有模型时才执行 shader 清理和 `ReloadAllRenderers()`，避免 `LoadAssets -> CreateRenderer` 崩溃。

#### 行为边界

- 正常后台/前台切换：应瞬间显示已加载模型，不出现 Lottie 或模型加载等待。
- 系统内存压力导致 EGL context 被回收：仍会走 `onSurfaceCreated()` 的 renderer/texture 重建兜底，这是 Android 层无法完全避免的慢路径。
- 应用进程被系统杀死或用户清除后台：重新启动必须重新加载模型。

#### 验证方式

1. 进入主界面等待模型正常显示。
2. 按 Home 或切到其他应用，等待一段时间。
3. 返回应用，预期 Live2D 模型立即显示。
4. logcat 中正常返回路径不应重复出现完整 `LoadAssets()` 模型加载链路；若系统回收 EGL context，允许出现 `onSurfaceCreated()` 与 renderer/texture 重建日志。
